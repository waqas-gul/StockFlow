import { z } from 'zod'
import { formatMoney, parseMoney, type DecimalParseError } from '@shared/domain'
import {
  MAX_BASE_QTY,
...
function optionalText(max: number): z.ZodType<string | null, string> {
  return z
    .string()
    .trim()
...
function moneyText(minorDigits: number): z.ZodType<number | null, string> {
...
function moneyMessage(error: DecimalParseError, minorDigits: number): string {
  switch (error) {
...
function wholeNumberText(min: number, max: number): z.ZodType<number, string> {
