...
import {
  ...
  PAYMENT_NOTE_MAX,
  type PaymentDetail as PaymentDetailData
...
import { paymentQuery, refreshAfterCustomerChange } from '@renderer/lib/app-queries'
import { formatDateTime } from '@renderer/lib/format'
import { formatAmount, type CurrencyFormat } from '../products/product-display'
import { submitPaymentVoid, type PaymentNotifier } from './payment-actions'
import { paymentCustomerLabel, voidConfirmationText } from './payment-display'
...
/** A saved payment. There is no edit: a mistake is voided. */
export function PaymentDetailDialog({
  ...
  const payment = useQuery({ ...paymentQuery(paymentId ?? 0), enabled: paymentId !== null })
  return (
    <Dialog open={paymentId !== null} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-[min(40rem,calc(100%-2rem))]">
        <DialogHeader>
          <DialogTitle>
            {payment.data ? `Payment ${payment.data.paymentNo}` : 'Payment'}
          </DialogTitle>
...
function VoidPayment({
  ...
  const voidIt = async (): Promise<void> => {
    setBusy(true)
    const voided = await submitPaymentVoid(
      window.api.payments,
    ...
    setConfirming(false)
    if (voided !== null) {
      queryClient.setQueryData(paymentQuery(payment.id).queryKey, voided)
      onVoided?.()
    }
  ...
  return (
    <div className="flex flex-col gap-2 rounded-md border p-3">
      <p className="text-sm text-muted-foreground">
        A saved payment is never edited. If it is wrong, void it with a reason and record the
        correct payment again.
      </p>
      <div className="flex flex-wrap items-end gap-2">
        <div className="grid min-w-64 flex-1 gap-1.5">
          <Label htmlFor="paymentVoidReason">Reason for voiding</Label>
          <Input
            id="paymentVoidReason"
      ...
      {confirming && (
        <div className="flex flex-col gap-2 rounded-md border border-destructive/30 bg-destructive/5 p-3">
          <p className="text-sm">{voidConfirmationText(payment, currency)}</p>
